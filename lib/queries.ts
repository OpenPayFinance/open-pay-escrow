...
export const useAdminDeliverFundMutation = () => {
	const queryClient = useQueryClient();
	const { escrowContract, signerAddress } = useContext(EscrowContext);
	return useMutation(
		["deliverByAdmin", signerAddress],
		(escrowId: BigNumber) => {
			if (!escrowContract) return Promise.resolve([]);
			const promise1: Promise<TransactionResponse> =
				escrowContract.deliverByAdmin(escrowId);
			const promise2: Promise<TransactionReceipt> = promise1.then(
				(trx: TransactionResponse) => trx.wait()
			);
			return toastQuery<Array<TransactionResponse | TransactionReceipt>>(
	...
	return useMutation(
		...
		(escrowId: BigNumber) => {
			...
			return toastQuery<Array<TransactionResponse | TransactionReceipt>>(
				Promise.all([promise1, promise2]),
				{
					loading: "Raising a Dispute",
					success: "Dispute Raised Successfully!, Please raise a ticket in support system.",
					error: (e: any) => `Error disputing: ${e.message}`,
				}
			);
		},
		{
...
export const useRefundByAdminMutation = () => {
	const queryClient = useQueryClient();
	const { escrowContract, signerAddress } = useContext(EscrowContext);
	return useMutation(
		["refundByAdmin", signerAddress],
		(escrowId: BigNumber) => {
			if (!escrowContract) return Promise.resolve([]);
			const promise1: Promise<TransactionResponse> =
				escrowContract.refundByAdmin(escrowId);
			const promise2: Promise<TransactionReceipt> = promise1.then(
				(trx: TransactionResponse) => trx.wait()
			);
			return toastQuery<Array<TransactionResponse | TransactionReceipt>>(
