    ...
    UUPSUpgradeable
{
    ...
    uint256 private constant _minimumEscrow = 0.01 ether;
        ...
        nonReentrant
    {
        ...
        idToEscrow[_escrowId].clearAt = block.timestamp;
        idToEscrow[_escrowId].state = EscrowState.IN_DISPUTE;
    ...
    function deliverByAdmin(uint256 _escrowId)
        public
        onlyOwner
        nonReentrant
    {
        require(
            idToEscrow[_escrowId].state == EscrowState.IN_DISPUTE,
            "You can't deliver this escrow. Only Owner can perform this action."
        );
    ...
    function refund(uint256 _escrowId) public notBuyer(_escrowId) nonReentrant {
        require(
            idToEscrow[_escrowId].state == EscrowState.AWAITING_DELIVERY || idToEscrow[_escrowId].state == EscrowState.IN_DISPUTE,
            "Can't refund this escrow."
        );
