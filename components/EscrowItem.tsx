...
import {
	...
	useEscrowMetaQuery,
	useRefundMutation,
	useAdminDeliverFundMutation,
	useRaiseDisputeMutation,
	useRefundByAdminMutation,
} from "../lib/queries";
...
export const EscrowItem = ({
	...
	const { mutate: mutateRaiseDispute } = useRaiseDisputeMutation();
	const { mutate: mutateAdminDeliverFund } = useAdminDeliverFundMutation();
	const { mutate: mutateRefundByAdmin } = useRefundByAdminMutation();
	...
	const deliverByAdmin = useCallback(() => {
		if (confirm("Are you sure to deliver fund?")) {
			mutateAdminDeliverFund(escrow.id);
		}
	}, [escrow.id, mutateAdminDeliverFund]);
	...
	const refundByAdmin = useCallback(() => {
		if (confirm("Are you sure to refund?")) {
			mutateRefundByAdmin(escrow.id);
		}
	}, [escrow.id, mutateRefundByAdmin]);
	
	const raiseDispute = useCallback(() => {
		if (confirm("Are you sure to raise a dispute?")) {
			mutateRaiseDispute(escrow.id);
		}
	}, [escrow.id, mutateRaiseDispute]);
	...
	return (
		...
		>
			...
			<ul className="list-group mt-3">
				...
				{escrow.state !== EscrowState.AWAITING_DELIVERY && escrow.state !== EscrowState.IN_DISPUTE && (
					<li className="list-group-item d-flex justify-content-between">
				...
				{escrow.state === EscrowState.IN_DISPUTE && (
					<li className="list-group-item d-flex justify-content-between">
						<span className="font-weight-bold">Dispute Raised At</span>
						<span>{toDate(escrow.clearAt).toDateString()}</span>
					</li>
				)}				
				{escrow.state === EscrowState.AWAITING_DELIVERY &&
